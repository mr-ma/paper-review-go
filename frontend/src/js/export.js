function exportHTML ( fileName ) {
  ...
  if (fullHTMLTmp.length <= 1) {
    handleError('Error occurred while extracting HTML from page.');
    return;
  }
  fullHTML = fullHTMLTmp[0] + DELIMITER + fullHTMLTmp[1] + DELIMITER;
  ...
  /*
        for ( var i = 0; i < links.length; i++ ) {
          ...
          console.log(links[i])
          if (!!webURL) {
            console.log("replacing href: " + srcURL + " with " + webURL + ': ' + new RegExp('<link href="' + srcURL + '" type="text/css" rel="stylesheet" data-web="' + webURL + '"/>', 'g'))
